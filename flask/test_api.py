...
def test_register_user():
    ...
    index = 1
    response = requests.post(LOCALHOST + path, headers=headers, json=test_users[index])
...
def test_add_expense():
    ...
    index = 2
    response = requests.post(LOCALHOST + path, headers=headers, json=test_expenses[index])
    assert response.status_code == 201
    # pprint(response)
